...
let
  ...
  signedFlashScript = writeShellApplication {
    name = "flash-signed-${cfg.name}";
    runtimeInputs = [
      coreutils
    ...
    text = ''
      ...
      usage() {
        cat <<'USAGE'
      Usage: flash-signed-${cfg.name} [options] [-s <signed-sd-image>]
...
in
{
  ...
  inherit extractSignedOrinArtifacts signedFlashScript;
